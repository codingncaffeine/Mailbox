...
/// <summary>
/// The two keyless ciphers a file can wrap its data blocks in ([MS-PST] §5.1, §5.2).
/// </summary>
/// <remarks>
/// Obfuscation, not encryption — the specification's own security section says as much — but
/// every byte of message data in most real files passes through here, so the tables have to be
/// exact. They are §5.1's <c>mpbbCrypt</c>: three 256-byte permutations in one array, the first
/// (R) encodes, the third (I) is its inverse and decodes, and the middle (S) belongs to the
/// cyclic cipher. Only external data blocks are ever encoded; pages and internal blocks —
/// XBLOCKs, subnode blocks — are always plain, and the trailer CRC is computed over the bytes
/// as stored, so it is checked before decoding.
/// </remarks>
internal static class BlockEncoding
{
    // R at 0, S at 256, I at 512 — §5.1's mpbbR / mpbbS / mpbbI in their published order.
    private static ReadOnlySpan<byte> Tables =>
    [
    ...
    /// <summary>Undoes NDB_CRYPT_PERMUTE in place — §5.1 with fEncrypt false, one table look-up per byte.</summary>
    public static void DecodePermute(Span<byte> data)
    {
        var inverse = Tables[512..];
        for (var i = 0; i < data.Length; i++)
            data[i] = inverse[data[i]];
    }

    /// <summary>The forward permutation. A reader never needs it; the round-trip test does.</summary>
    internal static void EncodePermute(Span<byte> data)
    {
        var forward = Tables[..256];
        for (var i = 0; i < data.Length; i++)
            data[i] = forward[data[i]];
    }

    /// <summary>
    /// NDB_CRYPT_CYCLIC, which is its own inverse — §5.2's exact walk, byte position folded into
    /// the key so identical runs encode differently. The key is the low 32 bits of the block's id,
    /// which is why a cyclic-encoded block cannot be checked or decoded without knowing which
    /// block it is.
    /// </summary>
    public static void Cyclic(Span<byte> data, uint key)
    {
        var tables = Tables;
        var w = (ushort)(key ^ (key >> 16));
        foreach (ref var slot in data)
        {
            var b = slot;
            b = (byte)(b + (byte)w);
            b = tables[b];
