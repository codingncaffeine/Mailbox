...
| `host.Pipeline.OnArrival` | `arrival` | Runs on every message as it arrives — last, after the junk filter, ignored conversations, the Focused Inbox, the rules and the newsletter router, so it sees where all of those put the message. Background thread. Answer `None`, `MoveTo(folderName)` or `Delete`; a name that is no folder is logged and the message stays put. |
